...
vectorstore = PineconeVectorStore(embedding=embedding, index_name="langchain-doc-index")
...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    try:
        ...
        log_warning("No existing vectors to delete")
    ...
    async def add_batch(batch: List[Document], batch_num: int):
        try:
            ...
            log_success(f"VectorStore Indexing: Successfully added batch {batch_num}/{len(batch)} documents")
...
async def main():
    ...
    all_docs = [Document(page_content=result['raw_content'], metadata={"source": result['url']}) for result in results]
